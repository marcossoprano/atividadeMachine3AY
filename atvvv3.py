...
DATA_FILE = Path("./barrettII_eyes_clustering.xlsx") # Fixed: __file__ is not defined in Colab notebooks
...
def print_cluster_report(df: pd.DataFrame, labels, scaler):
    df = df.copy()
    df["Grupo"] = labels
    ...
    print(f"Grupos usados para caracterização: {FINAL_K}\n")
    ...
    print("\nInterpretação dos perfis de olhos (baseada nas variáveis AL, ACD, WTW, K1 e K2):")
    for group in sorted(df["Grupo"].unique()):
        mean_values = df.loc[df["Grupo"] == group, FEATURES].mean()
        print(
            f"Grupo {group}: AL={mean_values['AL']:.2f}, ACD={mean_values['ACD']:.2f}, "
            f"WTW={mean_values['WTW']:.2f}, K1={mean_values['K1']:.2f}, K2={mean_values['K2']:.2f}"
        )
...
def main():
    ...
    print("Avaliação de K (2 a 7):")
    print(scores.to_string(index=False, float_format="%.4f"))

    print(f"\nEscolhendo K={FINAL_K} como melhor equilíbrio entre qualidade e interpretabilidade.")
    model, labels = fit_final_model(X_scaled, FINAL_K)
    print_cluster_report(df, labels, scaler)
